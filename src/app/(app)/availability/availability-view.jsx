...
export default function AvailabilityView({ initialSlots = [] }) {
  const router = useRouter()
  const [saving, setSaving] = useState(false)

  const handleSave = async () => {
    setSaving(true)
    const slots = selectedDates.map((day) => ({
      dayOfWeek: day % 7,
      startTime: startTime.replace(' AM', ':00').replace(' PM', ':00'),
    ...
    <div className="space-y-8">
      ...
      {isDesktop && (
        <section className="flex justify-between items-end gap-8">
          ...
          <PrimaryButton icon={saving ? undefined : 'check'} className="w-auto px-8" onClick={handleSave}>
      ...
      {!isDesktop && (
        <div className="fixed bottom-20 left-0 w-full p-6 bg-white/90 backdrop-blur-2xl z-40">
          <PrimaryButton onClick={handleSave}>{saving ? 'Saving...' : 'Save Availability'}</PrimaryButton>
        </div>
      )}
    </div>
  )
